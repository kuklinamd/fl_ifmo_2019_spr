...
typeCheckByName :: Ast -> Name -> Either String (Maybe Type)
typeCheckByName ast name = (lookup name) <$> typeCheckerCtx ast

typeCheckerCtx :: Ast -> Either String NameCtx
typeCheckerCtx ds = typeChecker' [] [] ds

typeCheckerCtx' :: Ast -> Either String (DataCtx, NameCtx)
typeCheckerCtx' ds = typeChecker'' [] [] ds

typeChecker' :: DataCtx -> NameCtx -> [Decl] -> Either String NameCtx
typeChecker' d n = fmap snd . typeChecker'' d n

typeChecker'' :: DataCtx -> NameCtx -> [Decl] -> Either String (DataCtx, NameCtx)
typeChecker'' d c [] = Right (d, c)
typeChecker'' dCtx fCtx (d : ds)
  | DataDecl dt@(Data name ctrs) <- d
  , Nothing <- lookup name dCtx
  = let ext = (ctrToType name <$> ctrs)
    in  typeChecker'' (ext ++ dCtx) (ext ++ fCtx) ds
  | DataDecl (Data name _) <- d
  , Just _ <- lookup name dCtx
  = Left $ "Data type '" ++ name ++ "' has already been declared!"

  | TypeDecl name tp <- d
  , Nothing <- lookup name fCtx
  = typeChecker'' dCtx ((name, tp) : fCtx) ds

  | TypeDecl name _ <- d
  , Just _ <- lookup name fCtx
  = Left $ "Type of function '" ++ name ++ "' has already been declared!"

  | BindDecl bind@(Bind name _ _) <- d
  , Just tp <- lookup name fCtx
  = do
    ctx <- checkBind dCtx fCtx tp bind
    typeChecker'' dCtx ctx ds

  | otherwise
  = Left "Function declared without its type!"

checkBind :: DataCtx -> NameCtx -> Type -> Bind -> Either String NameCtx
checkBind dCtx nCtx tp (Bind _ pats expr) =
  let (patTypes, resultType) = destructType tp
  in
    if length patTypes == length pats
      then do
        ...
        if disjoint ctxs
          then do
            ...
            if exprt /= resultType
              then
                Left
                $  "Result type and actual expr type doesn't match: ("
                ++ pretty exprt
                ++ ") and ("
                ++ pretty resultType
                ++ ")"
...
checkPat _ ctx expectedType (PatVar x)
  | Just tp <- lookup x ctx
  = Left $ "Name '" ++ x ++ "' is already bound: '" ++ pretty tp ++ "'!"
  | otherwise
  = Right [(x, expectedType)]
checkPat _ ctx TInt  (PatLit (ILit _)) = Right []
checkPat _ ctx TBool (PatLit (BLit _)) = Right []
checkPat dCtx fCtx expectedType (PatCtr n ps) | Just t <- lookup n dCtx = do
  let (argTypes, resType) = destructType t
  if resType == expectedType
    then if length argTypes == length ps
      then do
        ctxs <- mapM (uncurry (checkPat dCtx fCtx)) (zip argTypes ps)
        if disjoint ctxs
          then Right (concat ctxs)
          else Left $ "Boo3: " ++ show ctxs
      else
        Left
        $  "Different number of arguments in the constructor '"
        ++ n
        ++ "' and in its type."
    else
      Left
...
checkExpr dCtx fCtx (UnOp op e) = do
  ...
  case op of
    Minus -> if t == TInt
      then Right TInt
      else Left $ "Expression is not of integer type!"
    Neg -> if t == TBool
      then Right TBool
      else Left $ "Expression is not of boolean type!"
checkExpr dCtx fCtx (BinOp op e1 e2) = do
  t1 <- checkExpr dCtx fCtx e1
  t2 <- checkExpr dCtx fCtx e2
  case op of
    AO _ -> if t1 == TInt && t2 == TInt
      then Right TInt
      else Left "Expressions for arith operations aren't of integer type!"
    LO _ -> if t1 == TBool && t2 == TBool
      then Right TBool
      else Left "Expressions for arith operations aren't of boolean type!"
    CO _ -> if (t1 == TInt && t2 == TInt) || (t1 == TBool && t2 == TBool)
      then Right TBool
      else Left
        "Expressions for arith operations aren't of integer of boolean type!"


disjoint :: Eq a => [[a]] -> Bool
disjoint = fst . foldr
  (\n (s, acc) -> (s && (null (intersect n acc)), n ++ acc))
  (True, [])
...
destructTypeHead :: Type -> (Type, [Type])
destructTypeHead TInt          = (TInt, [])
destructTypeHead TBool         = (TBool, [])
destructTypeHead (TData a    ) = (TData a, [])
destructTypeHead (Arrow t1 t2) = let (t, ts) = destructTypeHead t2 in (t1, t : ts)
